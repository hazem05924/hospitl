        ...
        <div class="kt-subheader  kt-grid__item" id="kt_subheader">
            <div class="kt-container  kt-container--fluid ">
                <div class="kt-subheader__main">
                    ...
                    <h3 class="kt-subheader__title">جدول تقارير المختبر</h3>
                    ...
                    <table class="table table-striped- table-bordered table-primary text-center table-checkable kt_table_1">
                        ...
                        <tbody>
                            @foreach ($lapreports as $lapreport)
                                <tr>
                                    <td>{{ $lapreport->patient_id }} </td>
                                    <td>{{ $lapreport->doctor->first_name }} {{ $lapreport->doctor->last_name }}</td>
                                    <td>{{ $lapreport->template->name }}</td>
                                    <td>{{ $lapreport->date . '/' . $lapreport->time }}</td>
                                    <td>{{ $lapreport->report }}</td>
                                    <td>
                                        <form action="{{ route('lapreports.edit', $lapreport->id) }}" method="post">
                                            @method('get')
                                            @csrf
